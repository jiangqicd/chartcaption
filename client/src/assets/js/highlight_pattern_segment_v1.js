...
export function highlight_pattern_segment(parameters) {
    const annotatedChart = parameters["annotatedChart"]
    ...
    const isTextAnnotationEnable = ref(true)
    const selectedDetailsAttr = ref("")
    const isDetailsEnable = ref(true)
    ...
    console.log(isTextAnnotationEnableList, isDetailsEnableList, selectedDetailsAttrList)

    if (i != annotationsList.length - 1) {
        isTextAnnotationEnableList.push(isTextAnnotationEnable)
        isDetailsEnableList.push(isDetailsEnable)
        selectedDetailsAttrList.push(selectedDetailsAttr)
    }
                ...
                menu_items.push({
                    ...
                    label: h('label', {
                        ...
                        h('input', {
                            ...
                            checked: isTextAnnotationEnable.value,
                            onInput: () => {
                                console.log('Checkbox state changed');
                            },
                            onchange: function () {
                                isTextAnnotationEnable.value = !isTextAnnotationEnable.value
                                if (this.checked) {
    ...
    if (pattern == "prediction") {
        ...
        add_details_annotation(svg, detail_data,data_X_Y_without_prediction, xScale, yScale, x_scale_keyword, y_scale_keyword, index, start, end, aggregation, selectedDetailsAttrList[i], x_name, y_name, 0)
    ...
    if (i == annotationsList.length - 1) {
        watch(() => [...isAnnotationsEnabled], (newItems, oldItems) => {
            ...
            let detail_data = [];
            let correlation_attr_data = [];
            let correlation_obj_data = [];
            let lag_attr_data = [];
            let lag_obj_data = [];
            let detail_attrs = [];
            newItems.forEach((item, indx) => {
                if (indx != annotationsList.length - 1) {
                    ...
                    if (item) {
                        ...
                        let annotations = annotationsList[indx]["annotations"]
                        ...
                        if ("detail_annotation" in annotations) {
                            ...
                            Object.keys(detail_sub_data[0]).forEach(key => {
                                if (key != x_name && key != y_name) {
                                    detail_attrs.push(key)
                                }
                            });
                            detail_sub_data.forEach(item_i => {
                                data_X_Y_without_prediction.forEach(item_j => {
                        ...
                        if ("correlation_annotation" in annotations) {
                                ...
                                obj_data.forEach((e, i) => {
                                    if (i >= parseInt(element["start"]) && i <= parseInt(element["end"])) {
                                        correlation_obj_data.push({
                                            "index": data_X_Y_without_prediction[i]["index"],
                                            [x_name]: data_X_Y_without_prediction[i][x_name],
                                            [y_name]: e,
                                            "corr": element["corr"]
                                        })
                                    }
                            ...
                            annotations["correlation_annotation"]["attr_attr"].forEach((element) => {
                                ...
                                data_X_Y_without_prediction.forEach((e, i) => {
                                    if (i >= parseInt(element["start"]) && i <= parseInt(element["end"])) {
                                        attr_data[element["attr"]].push({
                                            [x_name]: data_X_Y_without_prediction[i][x_name],
                                            [element["attr"]]: data_X_Y_without_prediction[i][element["attr"]],
                                            "index": data_X_Y_without_prediction[i]["index"],
                                            "corr": element["corr"]
                                        })
                                    }
        ...
        watch([selectedDetailsAttrList, isDetailsEnableList], () => {
            let detail_data = [];
            let correlation_attr_data = [];
            let correlation_obj_data = [];
            let lag_attr_data = [];
            ...
            isAnnotationsEnabled.forEach((item, indx) => {
                if (indx != annotationsList.length - 1) {
                    ...
                    if (item) {
                        ...
                        //获取detail_annotation数据
                        if ("detail_annotation" in annotations && isDetailsEnableList[indx].value==true) {
                            let detail_sub_data = JSON.parse(annotations["detail_annotation"]["data_details"]);
                            Object.keys(detail_sub_data[0]).forEach(key => {
                                if (key != x_name && key != y_name) {
                                    detail_attrs.push(key)
